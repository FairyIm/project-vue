          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(item,index) in spuSaleAttrList" :key="index">
                ...
                <dd changepirce="0" v-for="(color,index) in item.spuSaleAttrValueList" :key="index" :class="{active:color.isChecked == '1'}" @click="attrClick(color,item.spuSaleAttrValueList)">{{color.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <!-- autocomplete="off" -->
                <input  class="itxt" ref="itxt" v-model="goodsNum" @change='iptNumber'>
...
<script>
  ...
  export default {
    ...
    mounted(){
      this.$store.dispatch('getDetailList',this.$route.params.keyword)
    },
    computed:{
      ...mapGetters(['detailList','categoryView','skuInfo','spuSaleAttrList']),
      ...mapState({
    ...
    methods:{
      ...
      // 添加购物车
      async addshopcar(){
        // let res= await this.$store.dispatch('getAddShoppingCart',{skuId:this.$route.params.keyword,skuNum:this.goodsNum})
        // 要求在detail页面返回加入购物车是否成功信息，上边信息返回promise（因此需要再次发送异步请求），store里面处理返回res.code状态是否200的返回值，使用try、catch处理发送
        try {
          await this.$store.dispatch('getAddShoppingCart',{skuId:this.$route.params.keyword,skuNum:this.goodsNum})
          // 传参
          sessionStorage.setItem('CARTINFO',JSON.stringify(this.skuInfo))
          this.$router.push({name:'addcartsuccess',query:{goodsNum:this.goodsNum}})
        } catch (error) {
          alert(error.message)
        }
      }
          ...
          .choose {
            ...
            .cartWrap {
              .controls {
                ...
                /* 去掉number类型自带的加减按钮 */
                .itxt::-webkit-outer-spin-button,input::-webkit-inner-spin-button{
